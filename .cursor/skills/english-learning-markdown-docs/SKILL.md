...
2. **Conventions** (match this repo):
   ...
   - **Paired editions (optional):** When two notes cover the same scene at different levels (e.g. **adult** vs **parent–child**), add shared YAML `tags` (e.g. `topic/emotions`) plus audience tags (`audience/adult`, `audience/parent-child`), and link both directions in the body.
   - **Folder MOC (`README`):** For a dense subtree (e.g. [[learning-notes/parenting-english/README|parenting-english/README]]), maintain a hub that lists every note; each child file includes one **索引** line (wikilink back to that README) and YAML `tags` including `parenting-english` plus a bucket tag (`parenting/daily-life`, etc.).
