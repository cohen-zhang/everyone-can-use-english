...
Well, the pillowcase, “pillowcase,” (one word) **is the thing that** you put over the pillow so the pillow doesn’t get dirty; **we call that** the pillowcase. I say, “Well, at least the sheets match.” We already know what a sheet is. **When we say** the sheets match, “match,” **we mean that** they are the same color or two colors that look good together. **We use that verb when** we are talking about clothing as well, or anything where you have two colors.
...
That concludes part one of “A Day in the Life of Jeff.” In part two, we’ll be “Cleaning Up.”
...
- **get up** — 起床；从床上起来。
- **wake up** — 醒来；结束睡眠。
- **handle** — 应付；能扛住；能处理（与 *deal with* 近义）。
- **covers** — 床上盖毯、被等的总称。
- **comforter** — 厚被子（常羽绒或保暖填充物）。
- **light blanket** — 薄毯。
- **pillow** — 枕头。
- **pillowcase** — 枕套。
- **sheets** — 床单；床单套装（含 *fitted sheet* 床笠式、*flat sheet* 平铺被单等）。
- **match** — （颜色、图案）相配。
- **go off**（alarm）— （闹钟）响。
- **jarring** — （声音）刺耳、令人不适。
- **buzzer** — 蜂鸣器；闹钟的蜂鸣声。
- **drive (someone) nuts** — 把某人逼疯、烦死。
- **wake-up call** — （酒店）电话叫醒服务。
- **early riser** — 习惯早起的人。
- **oversleep** — 睡过头。
- **sleep in** — 睡懒觉；比平常起得晚。
...
- **as I was saying** — 言归正传；接回刚才的话题（打断后回到原话题）。
- **slam**（door）— 砰地关（门）。
- **gunfire** — 枪声（文中为玩笑）。
- **Man, …** — 口语，表感叹、强调。
- **set the alarm** — 设定闹钟。
- **jump out of bed** — 一跃下床。
- **take on the world** — 精神抖擞迎接一天；准备大干一场。
- **one foot on the floor at a time** — 一只脚一只脚先着地（形容慢吞吞起床）。
...
**本课里反复出现的骨架举例**：*… means to …* · *… means that …* · *… is something that …* · *Another expression would be …* · *… is the same … as …* · *When we say …, we mean …* · *X is what / when …* · *The verb is to …* · *It’s an expression we use to say …* · *For example, …*

下面按**功能**归类（与课文中具体词无关，换别的词也会用同一套）。

### **… means to … / … means that … / here means …**

- **在干什么**：给动词、名词下定义（词典式释义）。
- **更简单说法**：*If you X, you …* / *“X” is when …* / *In this sentence, “handle” means …*
- **中文**：「……的意思是……」「在这里指……」

### **… is something that … / … are the things that …**

- **在干什么**：用定语从句说明 *something / things* 是什么（归类 + 特征）。
- **更简单说法**：*X is a thing / an action where …* / *X = things you use to …*
- **中文**：「……就是那种……的东西」

### **Another expression would be … / You can also say …**

- **在干什么**：提供**同义换说**（并列另一种说法）。
- **更简单说法**：*Same idea: …* / *Another way to say it: …*
- **中文**：「也可以说……」「另一个说法是……」

### **… is the same, in this case, as …**

- **在干什么**：说两个词/说法在**当前语境里**意思一样。
- **更简单说法**：*Here, “deal with” = “handle.”* / *They mean the same here.*
- **中文**：「在这句里，A 和 B 一个意思」

### **When we say X, we mean that …**

- **在干什么**：说明**日常说法**（phrase）在语用上的意思。
- **更简单说法**：*If people say “the alarm goes off,” they mean …*
- **中文**：「当我们说……时，指的是……」

### **X is what you … / X is when … / X is when someone …**

- **在干什么**：用 *what / when* 从句下定义（名词性或情景性释义）。
- **更简单说法**：*A pillow is for your head when you sleep.*
- **中文**：「X 就是你……的时候用的/发生的事」

### **… would be one that … / … would be the opposite**

- **在干什么**：在若干可能里**举例或对比**（which one / opposite）。
- **更简单说法**：*For example, a jarring noise is …* / *The opposite is …*
- **中文**：「就是那种……」「反过来则是……」

### **The verb is to … / We use that verb when …**

- **在干什么**：点明词性、使用场合。
- **更简单说法**：*We use “wake up” as a verb.* / *We say “match” for colors.*
- **中文**：「这是个动词」「我们在说……时用这个词」

### **We call that … / People say …**

- **在干什么**：交代**习惯说法、名称**。
- **更简单说法**：*The name for that is …*
- **中文**：「大家把这叫……」

### **It’s an expression we use to say that …**

- **在干什么**：说明**习语、固定表达**的整体功能。
- **更简单说法**：*“Take on the world” means you feel ready for the day.*
- **中文**：「这是一句固定说法，用来表达……」

### **The word X comes from …**

- **在干什么**：词源（常接 *verb / Latin* 等）。
- **更简单说法**：*“Riser” is related to “rise.”*
- **中文**：「这个词是从……变来的」

### **For example, …**

- **在干什么**：举例（常接具体时间、情景）。
- **更简单说法**：*Like this: …* / *Say you …*
- **中文**：「比方说……」

### **meaning … / I’m just kidding / that isn’t actually true**

- **在干什么**：补充说明上一句**真实含义**，或标明**玩笑**。
- **更简单说法**：*What I really mean is …* / *I’m joking.*
- **中文**：「意思是……」「我开玩笑的」
