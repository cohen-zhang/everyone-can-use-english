...
**相关（相向互链）：**
- [[learning-notes/tv-series/a-day-in-the-life-of-jeff/episode-notes/jeff-e04-getting-dressed-ready-for-work-notes|Episode 4 – Getting Dressed & Ready for Work]] — 上一集：穿衣准备上班
- [[learning-notes/tv-series/a-day-in-the-life-of-jeff/episode-notes/jeff-e06-desk-break-lunch-notes|Episode 6 – At My Desk, on Break, and at Lunch]] — 下一集：工位、休息与午餐
...
- [[learning-notes/personal-english-book/one-minute-drill/a-weekday.md|A Weekday · 工作日的一天]] — 个人一日流水：e-bike 送娃、地铁、共享单车

---

## Introduction

Welcome to ESLPod.com's "A Day in the Life of Jeff," part five: The Commute to Work. I'm Dr. Jeff McQuillan, your host.

This part of our course is going to talk about going to work and vocabulary related to commuting. Let's get started.
...
- **commute** — 通勤（名词/动词）；家与工作地点之间的往返。
- **back seat** — 后座（后排乘客座位）。*driver's seat* = 驾驶座；*passenger side* = 副驾驶一侧。
- **drive**（名词）— 一段驾车行程（与动词 *drive* 同形）。
- **coffee mug** — 大杯马克杯（装咖啡）。
- **cup holder** — 杯架（车内放杯/瓶的圆孔位）。
- **ignition** — 点火装置（钥匙插入启动发动机处）。*to ignite* = 点燃。
- **gas pedal** — 油门踏板。*pedal* 也可作动词（骑自行车 *pedal*）。
- **rear view mirror** — 车内后视镜（挡风玻璃上方中间）。
- **side mirror(s)** — 车外侧后视镜。
- **windshield** — 挡风玻璃。

### Starting & driving

- **drive time (traffic report)** — 高峰通勤时段（早晚上下班）及电台路况播报。
- **transmission** — 变速箱；传动系统。*gears* = 档位。
- **out of park / into park** — 把挡位从驻车挡（P）拨出 / 挂入驻车挡。
- **reverse** — 倒车挡；向后行驶。*to go into reverse* = 挂倒挡后退。
- **back out** — 倒车驶出（如从车库倒出）。
- **driveway** — 私家车道（连接街道与车库/家门的车道）。
- **put the car into drive** — 挂前进挡（D 挡）。
- **stick shift / manual transmission** — 手动挡。*automatic transmission* = 自动挡。
- **clutch** — 离合器踏板（手动挡第三踏板）。
- **brake pedal** — 刹车踏板。

### Freeway & arrival

- **freeway** — 高速公路（美式）。
- **onramp** — 上高速的匝道。反义：*off ramp* 下高速匝道。
- **get off**（freeway）— 下高速。
- **exit** — 出口；下高速处（亦可作动词「离开」）。
- **parking garage** — 停车楼（多层停车场）。
- **key card** — 门禁卡（刷卡进出车库/大楼）。
- **swipe** — 刷卡（快速划过读卡器）。
- **head toward** — 朝……走去/开去。
- **elevator** — 电梯。
...
**本课里反复出现的骨架举例**：*… means to …* · *… is what …* · *… is the part of …* · *… comes from …* · *notice we use …* · *… is when …* · *… is the same as …* · *The opposite of X would be …* · *… is used when …* · *In this case, it means …*
...
- **在干什么**：给动词、短语下定义。
- **更简单说法**：*To commute means to go from home to work.*
- **中文**：「……的意思是……」

### **… is what … / … is the part of …**

- **在干什么**：说明物品、部件是什么、干什么用。
- **更简单说法**：*The ignition is the part that starts the engine.*
- **中文**：「……就是……」「是汽车的……部分」

### **… comes from … / That prefix re, means …**

- **在干什么**：词源或前缀说明（如 *re-* = again，*ignite* → *ignition*）。
- **更简单说法**：*"Re-adjust" = adjust again.*
- **中文**：「这个词来自……」「前缀 re- 表示再……」

### **notice we use … / … can be both a noun and a verb**

- **在干什么**：点明词性、同一词的不同用法（如 *drive* 名/动，*commute* 名/动）。
- **更简单说法**：*Here "drive" is a noun, not a verb.*
- **中文**：「注意这里当……用」「既可作名词也可作动词」

### **… is when … / … is the time of day when …**

- **在干什么**：用 *when* 从句说明情景或时段（如 *park*、*drive time*）。
- **更简单说法**：*Park is when the car isn't moving.*
- **中文**：「就是……的时候」

### **… is the opposite of … / The opposite of X would be …**

- **在干什么**：反义对照（*manual* ↔ *automatic*，*onramp* ↔ *off ramp*）。
- **更简单说法**：*The opposite is …*
- **中文**：「和……相反」「反过来是……」

### **… is the same as …**

- **在干什么**：等同说法（*stick shift* = *manual transmission*）。
- **更简单说法**：*Same thing: …*
- **中文**：「就等于……」

### **… is used when … / … is the verb we use when …**

- **在干什么**：说明使用场合（如 *back out* 用于倒车驶出，*swipe* 用于刷卡）。
- **更简单说法**：*We say "back out" for leaving a garage in reverse.*
- **中文**：「在……情况下用这个词」

### **In this case, it means … / to put a car into drive means that …**

- **在干什么**：同一词在**当前语境**里的特定含义（*drive* 此处 = 前进挡）。
- **更简单说法**：*Here, "drive" means forward gear.*
- **中文**：「在这里指的是……」

### **That means to … / shift means to change**

- **在干什么**：动词释义（*shift* = 换挡/改变）。
- **更简单说法**：*To shift = to change gears.*
- **中文**：「意思是……」

---

*Source: ESLPod.com "A Day in the Life of Jeff," Episode 5. Markdown layout merges line breaks from the transcript for readability; glossary and full-course navigation are maintained in this notes set.*
