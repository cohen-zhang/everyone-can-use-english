...
*约 2 分钟 · 拟人口吻 · 按分工 → 同步 → 互斥展开 · 先英后对照简中*
...
Without coordination, thread interleaving is unpredictable. You get **race conditions** — wrong results that depend on timing. You get **visibility** failures — one thread never sees another's write. You get **deadlock** — threads wait in a circle, each holding what the other needs. Those are symptoms. Underneath, I only have three jobs: **division of labor**, **synchronization**, and **mutual exclusion**. Most of `java.util.concurrent` is organized along these three lines. Do not hand-roll thread safety — pick the tool that matches the job.

**简中：** 缺少协调时，线程交错不可预测。会出现**竞态条件**——结果随时机而错。会出现**可见性**失效——一条线程看不见另一条的写入。会出现**死锁**——线程循环等待，各自握着对方需要的东西。那些是症状。底下我只有三件事：**分工**、**同步**、**互斥**。`java.util.concurrent` 很大一部分就是按这三条线组织的。不要手写线程安全——按问题选工具。

**Division of labor** comes first: split the work and assign it to threads. Do not create a raw thread for every task. Submit work to a **ThreadPoolExecutor** with an explicit queue, pool sizes, and a rejection policy. Pools **reuse** threads — creation is expensive. They **bound** concurrency so you do not exhaust memory. `CallerRunsPolicy` adds **back-pressure** when the queue is full. **Fork/Join** is the same idea for CPU-heavy work: recursively break a job into subtasks, then join the results. **CompletableFuture** composes async steps without blocking the caller: `supplyAsync` → `thenApply` → `thenCombine`.

**简中：** **分工**在先：把任务拆开，再交给线程。不要为每个任务裸建线程。把工作提交给显式配置了队列、池大小和拒绝策略的 **ThreadPoolExecutor**。线程池**复用**线程——创建代价高。它们**约束**并发，避免耗尽内存。队列满时 `CallerRunsPolicy` 提供**背压**。**Fork/Join** 是同一思路用在 CPU 密集活上：递归拆成子任务，再把结果会合。**CompletableFuture** 编排异步步骤，而不阻塞调用方：`supplyAsync` → `thenApply` → `thenCombine`。

Once the work is split, threads still have to wait for one another. That is **synchronization** — cooperation, not exclusion. A **CountDownLatch** is the typical case: one or more threads wait until N events finish, then they move on. A **CyclicBarrier** is reusable: a fixed group meets at the same point again and again. **BlockingQueue** is cooperation by handoff: `put` blocks when full, `take` blocks when empty. A **Semaphore** issues counting permits; it can wait, and it can cap how many threads enter. Pick the synchronizer that matches the wait pattern.

**简中：** 活拆开了，线程仍要彼此等待。那是**同步**——协作，不是互斥。**CountDownLatch** 是典型做法：一条或多条线程等到 N 个事件完成，再继续往下走。**CyclicBarrier** 可复用：固定一组线程反复在同一点会合。**BlockingQueue** 用交接来协作：满时 `put` 阻塞，空时 `take` 阻塞。**Semaphore** 发放计数许可；它可以等待，也可以限制同时进入的线程数。按等待模式选同步器。

After they cooperate, they still must not trample shared data. That is **mutual exclusion**: only one thread inside a **critical section** at a time. Prefer **immutability** and **confine** state first — no shared mutable data means no lock. When sharing is unavoidable, **`synchronized`** gives exclusion plus a **happens-before** edge: writes before unlock are visible after the next lock. **ReentrantLock** is the same means, with extras: `tryLock`, timed waits, interruptible acquisition, and fairness. Always **unlock** in a **`finally`** block. Miss that, and other threads wait forever. **ReadWriteLock** lets many readers proceed together; a writer excludes everyone. Use it on read-heavy caches.

**简中：** 协作之后，仍不能踩乱共享数据。那是**互斥**：同一时刻只允许一条线程进入**临界区**。先优先**不可变**并**封闭**状态——没有共享可变数据，就不需要锁。必须共享时，**`synchronized`** 提供互斥，并建立 **happens-before**：解锁前的写入，对下一次加锁可见。**ReentrantLock** 是同一类手段，但多了 `tryLock`、超时等待、可中断获取和公平性。永远在 **`finally`** 里 **unlock**。漏了这一步，其他线程会一直等。**ReadWriteLock** 允许多读并发，写则独占。适合读多写少的缓存。

**Volatile** is not a lock. It gives **visibility**, not **atomicity**. Use it for flags such as `running`. `count++` is three steps: read, add, write. Two threads can both read the same value and both write plus one. Compound actions need exclusion or an atomic class. **ConcurrentHashMap** shrinks the exclusion: it partitions the map, so most operations need no **global lock**. Do not share a plain `HashMap` across threads and patch a lock after the bug appears.

**简中：** **volatile** 不是锁。它保证**可见性**，不保证**原子性**。用在 `running` 这类标志上。`count++` 是读、加、写三步。两条线程可能读到同一个值，各自只加一。复合动作需要互斥或原子类。**ConcurrentHashMap** 把互斥范围缩小：对映射分区，多数操作无需**全局锁**。不要先让普通 `HashMap` 跨线程共享，出了 bug 再补一把锁。

You need me for I/O-bound services, parallel CPU work, shared caches, producer–consumer pipelines, and request spikes. Split the work, wait in the right place, and exclude only where state is shared. Follow that order and I raise throughput. Skip it and I produce races, stale reads, leaked locks, and pool exhaustion. The cost is not speed — it is correctness under interleaving.

**简中：** I/O 密集服务、可并行的 CPU 工作、共享缓存、生产者–消费者流水线、请求峰值，都需要我。先拆任务，再在对的地方等待，只在共享状态处互斥。按这个顺序，我提高吞吐。跳过它，就会出现竞态、读到旧值、锁泄漏和线程池耗尽。代价不是速度，而是交错执行下的正确性。
